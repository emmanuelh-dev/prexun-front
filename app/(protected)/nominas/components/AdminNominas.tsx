...
export default function AdminNominas() {
  ...
  const handleCreateSection = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: "No se pudo crear la sección", variant: "destructive" });
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleBatchUpload = async () => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error en la carga masiva", variant: "destructive" });
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleUserFileUpload = async (userId: number, file: File) => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Error al cargar archivo a usuario", variant: "destructive" });
    }
  };
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-4 items-end">
              <div className="flex-1 space-y-2">
                <label className="text-xs font-semibold text-muted-foreground">Semana / Periodo Activo</label>
                <SearchableSelect
                  options={secciones.map(s => ({ value: s.id.toString(), label: s.nombre }))}
              ...
              <div className="flex items-center gap-2">
                {!isCreatingNew ? (
                  <Button variant="outline" onClick={() => setIsCreatingNew(true)} className="dark:border-slate-700">
                  ...
                  <div className="flex gap-2 items-center bg-muted/50 dark:bg-slate-800/50 p-1 rounded-lg border dark:border-slate-700">
                    ...
                    />
                    <Button size="sm" onClick={handleCreateSection} disabled={uploading || !newSectionName}>
                      {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Crear"}
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setIsCreatingNew(false)}>Cancelar</Button>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex -space-x-2">
                    {selectedFiles.slice(0, 3).map((f, i) => (
                      <div key={i} className="h-8 w-8 rounded-full bg-white dark:bg-slate-800 border dark:border-slate-700 flex items-center justify-center shadow-sm">
                  ...
                  >
                    ...
                    Cargar {selectedFiles.length} {selectedFiles.length === 1 ? 'Archivo' : 'Archivos'}
                  </Button>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="overflow-hidden border-t-0 border-x-0 border-b-0 shadow-lg dark:border-slate-800">
            <Table>
              <TableHeader className="bg-slate-100/50 dark:bg-slate-800/50">
                <TableRow>
                  <TableHead className="w-[300px] text-xs font-bold uppercase">Empleado</TableHead>
                  <TableHead className="text-xs font-bold uppercase text-center">RFC</TableHead>
                  <TableHead className="text-xs font-bold uppercase text-center">Estado de Nómina</TableHead>
                  ...
                  return (
                    <TableRow key={user.id} className="hover:bg-muted/30 transition-colors group">
                      <TableCell className="py-3">
                        <div className="font-semibold text-neutral-800 dark:text-neutral-200">{user.name}</div>
                      ...
                      <TableCell className="text-right">
                        {!record ? (
                          <div className="flex justify-end items-center gap-2">
                            ...
                            />
                            <Button variant="ghost" size="sm" asChild className="h-8 text-neutral-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/40 opacity-0 group-hover:opacity-100 transition-opacity">
                              <label htmlFor={`user-file-${user.id}`} className="cursor-pointer">
